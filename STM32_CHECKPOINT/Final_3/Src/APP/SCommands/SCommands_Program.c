...
void SCommands_MoveFilterSelectMotor         (uint8* P_uint8MotorSteps)
{
	...
	if(P_uint8MotorSteps != NULL_PTR)
	{
		...
		if(Loc_uint32MotorSteps > Loc_uint32FilterCurrentSteps)
		{
			...
			HMOTOR_Step(HMOTOR_FILTER_SELECT, Loc_uint32MotorDiff, HMOTORS_CLKWISE_DIRECTION, 4);
		}
		else if(Loc_uint32MotorSteps < Loc_uint32FilterCurrentSteps)
		{
			...
			HMOTOR_Step(HMOTOR_FILTER_SELECT, Loc_uint32MotorDiff, HMOTORS_CCLKWISE_DIRECTION, 4);
		}
		else
		{
			/*Do nothing*/
...
float32 round_float(float32 number)
{
	if((number - (uint32)(number)) >= 0.5)
	{
		return ((uint32)(number) + 1.0f);
	}
	else
	{
...
void SCommands_ScanOperation			(void)
{
	...
	//	if((current_steps - (uint32)(current_steps)) >= 0.5f)
	//	{
	//		current_steps -= 0.5f;
	//		current_steps ++;
	//		skipped++;
