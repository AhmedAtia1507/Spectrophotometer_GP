...
#define  SPI_SR_TXE                          ((u16)0x02)               /*!< Transmit buffer Empty */
#define  SPI_SR_RXNE                         ((u16)0x01)               /*!< Receive buffer Not Empty */


void  MCAL_SPI_TX_RX (SPI_TypeDef *SPIx, u16 *pTxBuffer,enum PollingMechism PollingEn )
{
...
void MCAL_SPI_SendData	(SPI_TypeDef *SPIx, u16 *pTxBuffer,enum PollingMechism PollingEn )
{
	u8 index = 0 ;
	if (SPIx == SPI1)
	{
...
void MCAL_SPI_ReceiveData	(SPI_TypeDef *SPIx, u16 *pTxBuffer ,enum PollingMechism PollingEn )
{
	u8 index = 0 ;
	if (SPIx == SPI1)
	{
...
void MCAL_SPI_GPIO_Set_Pins (SPI_TypeDef *SPIx)
{
	if (SPIx == SPI1)
	{
		...
		if (Global_SPI_Config[SPI1_INDEX]->Device_Mode == SPI_Device_Mode_MASTER )
		{
			...
			case SPI_NSS_Hard_Master_SS_output_disable:
				...
				MGPIO_u8SetPinMode(GPIO_PORTA, GPIO_PIN4, GPIO_INPUT_FLOATING_MOD);
				break ;


			case SPI_NSS_Hard_Master_SS_output_enable:
				//Hardware master/ NSS output enabled Alternate function push-pull
				MGPIO_u8SetPinMode(GPIO_PORTA, GPIO_PIN4, GPIO_OUTPUT_AF_PUSH_PULL_10MHZ);
				break ;
			}
			...
			MGPIO_u8SetPinMode(GPIO_PORTA, GPIO_PIN5, GPIO_OUTPUT_AF_PUSH_PULL_10MHZ);


			//		PA6 : SPI1_MISO  (supported only full duplex)
			//			Full duplex / master Input floating

			MGPIO_u8SetPinMode(GPIO_PORTA, GPIO_PIN6, GPIO_INPUT_FLOATING_MOD);
			...
			MGPIO_u8SetPinMode(GPIO_PORTA, GPIO_PIN7, GPIO_OUTPUT_AF_PUSH_PULL_10MHZ);
		...
		}else  //Slave
		{
			...
			MGPIO_u8SetPinMode(GPIO_PORTA, GPIO_PIN5, GPIO_INPUT_FLOATING_MOD);
			...
			MGPIO_u8SetPinMode(GPIO_PORTA, GPIO_PIN6, GPIO_OUTPUT_AF_PUSH_PULL_10MHZ);

			//		PA7 : SPI1_MOSI
			//			Full duplex / slave Input floating
			MGPIO_u8SetPinMode(GPIO_PORTA, GPIO_PIN7, GPIO_INPUT_FLOATING_MOD);
	...
	}else if (SPIx == SPI2)
	{
		...
		if (Global_SPI_Config[SPI2_INDEX]->Device_Mode == SPI_Device_Mode_MASTER )
		{
			...
			case SPI_NSS_Hard_Master_SS_output_disable:
				//Hardware master /slave Input floating
				MGPIO_u8SetPinMode(GPIO_PORTB, GPIO_PIN12, GPIO_INPUT_FLOATING_MOD);
				break ;


			case SPI_NSS_Hard_Master_SS_output_enable:
				//Hardware master/ NSS output enabled Alternate function push-pull
				MGPIO_u8SetPinMode(GPIO_PORTB, GPIO_PIN12, GPIO_OUTPUT_AF_PUSH_PULL_10MHZ);
				break ;
			}
			...
			//		PB13 : SPI2_SCK
			//			Master Alternate function push-pull
			MGPIO_u8SetPinMode(GPIO_PORTB, GPIO_PIN13, GPIO_OUTPUT_AF_PUSH_PULL_10MHZ);


			//		PB14 : SPI1_MISO  (supported only full duplex)
			//			Full duplex / master Input floating

			MGPIO_u8SetPinMode(GPIO_PORTB, GPIO_PIN14, GPIO_INPUT_FLOATING_MOD);
			...
			//		PB15 : SPI1_MOSI
			//			Full duplex / master Alternate function push-pull
			MGPIO_u8SetPinMode(GPIO_PORTB, GPIO_PIN15, GPIO_OUTPUT_AF_PUSH_PULL_10MHZ);
		...
		}else  //Slave
		{
			...
			//		B13 : SPI1_SCK
			//			Slave Input floating
			MGPIO_u8SetPinMode(GPIO_PORTB, GPIO_PIN13, GPIO_INPUT_FLOATING_MOD);


			//		PB14 : SPI1_MISO  (supported only full duplex)
			//			Full duplex / slave (point to point) Alternate function push-pull
			//todo			Full duplex / slave (multi-slave) Alternate function open drain
			MGPIO_u8SetPinMode(GPIO_PORTB, GPIO_PIN14, GPIO_OUTPUT_AF_PUSH_PULL_10MHZ);
