                ...
                Note that in the interrupt priority register, the least significant 4 bits 
                    are reserved "You Can't write on them", so you have only the most significant 4 bits to write on
                                            ...
                                            |     Priority    ||||||||||||||||||                 
                                             -----------------------------------
        */
        u16 Loc_u16AIRCRPriorityGroupVal = (u16)(SCB_AIRCR & (0xFFFF));
        u8 Loc_u8PriorityValue = 0;
        ...
        switch(Loc_u16AIRCRPriorityGroupVal)
        {
            ...
            {
                Loc_u8PriorityValue = ((Copy_u8SubGroupPriority) | (Copy_u8GroupPriority << ((Loc_u16AIRCRPriorityGroupVal - (u16)NVIC_PRIORITY_GROUP_4_SUB_0) / 0x100)));
